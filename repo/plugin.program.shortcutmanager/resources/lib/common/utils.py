...
def wipe(folder=_addon_data, over=False):
    ...
    if not over:
        ...
        if not dialog.yesno("Shortcut Manager", get_string(30043)):
            del dialog
            return False
    ...
    if dialog:
        dialog.ok("Shortcut Manager", get_string(30168))
        del dialog
    return True
...
def set_color(setting=False):
    ...
    if value != -1:
        if value not in colors:
            if len(value) < 6:
                dialog.notification("Shortcut Manager", get_string(30111))
                del dialog
                return
...
def update_container(reload=False):
    ...
    if reload:
        if in_media:
            ...
            xbmc.executebuiltin("UpdateLibrary(video, Shortcut Manager)")
