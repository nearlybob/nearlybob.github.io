...
color_tag = r"\[\w+(?: \w+)*\](?:\[\w+(?: \w+)*\])?(\w+)(?:\[\/\w+\])?\[\/\w+\]"
plus = utils.get_art("plus")


def shift_group(group_id, target):
    anchor = manage.get_group_by_id(group_id)
    if not anchor:
        return

    # Only reorder against siblings (same parent, or other top-level
    # groups) so a subfolder doesn't get swapped against an unrelated
    # group elsewhere in the flat, global sort order.
    parent = anchor.get("parent")
    groups = [g for g in manage.find_defined_groups() if g.get("parent") == parent]
    group_def = None
    swap_def = None
...
def remove_group(group_id, over=False):
    if _remove_group(group_id, over):
        utils.update_container(True)


def remove_path(group_id, path_id, over=False):
    if _remove_path(path_id, group_id, over):
        utils.update_container(True)
...
def _remove_group(group_id, over=False):
    ...
    if not over:
        choice = dialog.yesno("Shortcut Manager", utils.get_string(30024))

    if over or choice:
        parent_id = group_def.get("parent")
        _remove_subfolder_tree(group_id)
        if parent_id:
            # Deleting a subfolder also removes its pointer from the parent
            # group's path list so the folder row does not linger.
            parent_def = manage.get_group_by_id(parent_id)
            if parent_def:
                parent_def["paths"] = [
                    p
                    for p in parent_def.get("paths", [])
                    if not (p.get("target") == "subfolder" and p.get("group") == group_id)
                ]
                manage.write_path(parent_def)
        dialog.notification(
            "Shortcut Manager", utils.get_string(30029).format(six.text_type(group_name))
        )
        del dialog
        return True
    del dialog
...
def _remove_subfolder_tree(group_id):
    # Recursively delete this group and any subfolders nested under it
    # (tracked via "subfolder" path entries) so removing a parent doesn't
    # leave orphaned .group files or dangling pointers behind.
    group_def = manage.get_group_by_id(group_id)
    if group_def:
...
def _remove_path(path_id, group_id, over=False):
    dialog = xbmcgui.Dialog()
    if not over:
        choice = dialog.yesno("Shortcut Manager", utils.get_string(30021))
...
def _show_options(group_def, path_def=None, type=""):
    ...
    if path_def:
        options.append(
            u"[COLOR firebrick]{}[/COLOR]".format(
                six.ensure_text(utils.get_string(30013))
            )
        )
    if not type:
        target = path_def["target"] if path_def else group_def["type"]
        if target not in ["shortcut", "settings"]:
            main_action = utils.get_string(30047)
        else:
            main_action = utils.get_string(30030)
    else:
    ...
    if idx < 0:
        return
    elif path_def and idx == len(options) - 1:
...
def edit_dialog(group_id, path_id=None, type=""):
    updated = False
